...
;; The big takeaway here is that freeze gives us giant speedups
(define (draw w)
  (define c (world-canvas w))
  (define ps (world-points w))
  (freeze
            ...
            (define row-image
              (freeze
               (foldl (λ (cidx canvas)
                        ...
                        (if (eq? pv 1)
                            (underlay/xy canvas (* cidx PIXEL-SIZE) 0 ON)
                            canvas))
                      row-canvas COLIDX)))
            (underlay/xy canvas 0 (* idx PIXEL-SIZE) row-image)) CANVAS ROWIDX)))

(define (main)
  (define w (big-bang (world (randomize-fields FIELD)
                 CANVAS)
          (on-tick tick)
          (to-draw draw)))
  0)
